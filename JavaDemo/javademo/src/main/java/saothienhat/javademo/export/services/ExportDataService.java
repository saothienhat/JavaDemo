package saothienhat.javademo.export.services;
...
//
//            System.out.println("Export done ! Path: " + fileLocation);
...
//    }
	...
	public ExportFileResult exportExcel(String fileName, String sheetName, List<String> headers, List<ExcelRow> rowDataList)  throws IOException{
		...
		try {
			...
			String fileLocation = this.getExportPath();
			if(fileLocation == null || fileLocation.isEmpty()) {
				File currDir = new File(".");
				String path = currDir.getAbsolutePath();
				fileLocation = path.substring(0, path.length() - 1) + fileName + ".xlsx";
			} else {
				fileLocation += fileName + ".xlsx";
			}
			
			System.out.println("===>  Excel file is exported at: " + fileLocation);
			...
			exportFileResult.setMessage("Export Done");
			exportFileResult.setFileName(fileName);
			exportFileResult.setExportLocation(fileLocation);
		} catch (IOException e) {
			System.err.println(e.getMessage());
			exportFileResult.setStatus("IOException");
		} finally {
